...
internal class BatchedSingleValue<T>(
    ConnectionContext? context,
    BatchedCommandKind kind
) : IBatchedValue<T>, IBatchItem
{
    ...
    public async Task<T> GetValueAsync(CancellationToken cancellationToken = default)
    {
        ...
        return kind switch
        {
            BatchedCommandKind.First => result.First(),
            BatchedCommandKind.FirstOrDefault => result.FirstOrDefault()!,
            BatchedCommandKind.Single => result.Single(),
            BatchedCommandKind.SingleOrDefault => result.SingleOrDefault()!,
            _ => throw new InvalidOperationException($"Unknown {nameof(BatchedCommandKind)}: '{kind}'."),
        };
    }
